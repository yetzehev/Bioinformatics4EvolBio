...
```{r}
# # Install and use marmap for bathymetry data
# if (!require(marmap)) {
#   install.packages("marmap")
#   library(marmap)
# }
# 
# # Get bathymetry data for your region
# bathy <- getNOAA.bathy(lon1 = min(coord_matrix[,1])-5, 
#                        lon2 = max(coord_matrix[,1])+5,
#                        lat1 = min(coord_matrix[,2])-5, 
#                        lat2 = max(coord_matrix[,2])+5,
#                        resolution = 4)


# Add your tess3 results
plot(q.matrix, coord_matrix, method = "map.max", add = TRUE)
```
