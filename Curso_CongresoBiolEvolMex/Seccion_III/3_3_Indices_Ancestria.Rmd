...
system("bcftools query -l ./subset/maices_subset.recode.vcf > ./subset/maices_subset_vcfOrder.txt")
system("vcftools --vcf ./subset/maices_subset.recode.vcf --plink --out ./subset/maices_subset")
```

2.  Se recomienda partir del formato ped a lfmm

```{r formato}
maices.geno <- ped2lfmm(input.file = "./subset/maices_subset.ped", output.file = "./subset/maices_subset.lfmm", force = TRUE)
head(maices.geno)
class(maices.geno)
```
...
sorted.IDs <- read.delim("./subset/maices_subset_vcfOrder.txt", header = FALSE, sep = "\t")
...
coordenadas <- read.delim("./meta/maices_subset_metadata.txt", head = TRUE, sep = "\t")
...
genotypes <- read.lfmm("./subset/maices_subset.lfmm")
dim(genotypes)
