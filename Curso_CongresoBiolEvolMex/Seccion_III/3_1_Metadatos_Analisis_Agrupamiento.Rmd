...
### Subconjunto de datos con 50 genotipos
...
**lista con 50 genotipos**, para ello usaremos la función de R
**`sample.`**

```{r subset_maices}
set.seed(123)  # For reproducibility
maices_slice <- maices[sample(nrow(maices), 50), ]
kable(head(maices_slice,5), align = "c") %>%
  kable_styling(full_width = FALSE)
...
```{r add.coord}
maices_meta <- merge(maices_slice, accesiones, by = "Accesion_ID", all.x = TRUE)
kable(head(maices_meta,5), align = "c") %>% 
  kable_styling(full_width = FALSE)

write.table(maices_meta, "./meta/maices_subset50_metadata.txt",
...
1.  El archivo para crear el subset del archivo ***VCF*** solo requiere
    ***unacolumna** con la lista de genotipos*.
