...
**lista con 72 genotipos**.

```{r subset_maices, echo= FALSE}

maices_subset <- read.delim("./meta/maices_subset.txt", header = FALSE, sep = "_")
colnames(maices_subset) <- c("Genotipo", "Accession_ID")
maices_slice <- maices[maices$Genotipo %in% maices_subset$Genotipo,]
...
write.table(maices_meta, "./meta/maices_subset_metadata.txt",
...
subset_vcf<- paste(maices_slice$Genotipo, maices_slice$Accesion_ID, sep = "_")
...
```{r}
system("vcftools --vcf ./../../data/Zea_mays.AGPv4.2x_0.8_0.01.NewID.Rojas2019.recode.vcf --keep ./meta/subset_maices_vcf.txt --recode --out ./subset/maices_subset")
system("bgzip -c ./subset/maices_subset.recode.vcf > ./subset/maices_subset.vcf.gz")
```
...
```{r transformacion}
#Filtrado de SNPs
system("~/bin/plink/plink --vcf ./subset/maices_subset.recode.vcf --keep ./meta/subset_maices_plink.txt --double-id --allow-extra-chr --set-missing-var-ids @:# --indep-pairwise 50 10 0.1 --out ./subset/maices_subset")
```
...
system("~/bin/plink/plink  --vcf  ./subset/maices_subset.recode.vcf --keep ./meta/subset_maices_plink.txt --double-id --allow-extra-chr --set-missing-var-ids @:# --extract ./subset/maices_subset.prune.in --make-bed --pca --out ./subset/maices_subset")
```
...
```{r graficar}
# read in data
pca <- read.delim("./subset/maices_subset.eigenvec", sep = " ", header = FALSE)
eigenval <- scan("./subset/maices_subset.eigenval")
```
