...
# 3.2 Análisis  exploratorios de agrupamiento con SNPs: PCA & DAPC

En la `Secccion II` editamos la tabla de los genotipos y agregamos los metadatos disponibles para cada gentipo. En esta seccion realizaremos un para de analisis exploratorios para visualizar los patrones de agrupamiento.
...
Para realizar el subconjunto de datos solo con los teocintles, cargaremos el archivo de texto que contiene la lista con los IDS
...
```{r eigenv_plot, include=TRUE}
# make plot
a <- ggplot(pve, aes(PC, pve)) + geom_bar(stat = "identity")
a + ylab("Percentage variance explained") + theme_light()
```

```{r eigenv_cumm, include=TRUE}
...
### Análisis de Discriminantes de Componentes Principales

El Análisis de Discriminantes de Componentes Principales (DAPC) es un método multivariante de análisis diseñado para identificar y describir grupos de individuos genéticamente relacionados [Referencia](https://doi.org/10.1186/1471-2156-11-94). DAPC utiliza K-means secuencial y selección de modelos para inferir grupos genéticos, el DAPC dará más importancia a las diferencias entre grupos que a las diferencias dentro de un mismo grupo.

El primer paso es cargar el archivo  `*vcf*` con el sunconjunto de teocintles y a partir de el generar un archivo `*genelight*` with vcfR. Posterirmente, extraer la lista de IDs del VCF para ordenar los metadatos de acuerdo al vcf
...
vcf tools requiere un archivo  separado por tabulador con el numero de cromosoma y el numero de variante a excluir. Para ello, editaremos el archivo de salida de plink con `sed -i` y ese archivo se empleara para realizar el filtrado con `vcftools`.
```{r vfc_filtrado}
...
El archivo vcf filtrado ahora puede cargarse en el entorno de Rstudio con el comando `vcfR`.
...
#Datos geograficos: ID, LATITUDE, LONGITUDE and SAMPLING LOCATION.
geo <- teocintles_meta_ordenado[, c("Genotipo", "Accesion_ID", "Latitude", "Longitude", "Locality")]


# Explore the genelight object
kable(head(ps.light@ind.names,5), align = "c") %>% 
  kable_styling(full_width = FALSE)

kable(head(ps.light@pop,5), align = "c") %>% 
  kable_styling(full_width = FALSE)
```

Durante la conversion de `*vcf*` a `*genelight*`, aquellos loci con mas de dos alelos fueron removidos. El archivo genelight ahora puede ser empleado para realizar el DAPC.

**Nota:** 60, 000 SNPs take ~30 min to run the PCA. Apliqué filtros muy extrictos para conservar un set final de 11,907 SNPs

```{R ps_dapc}

# 1. Cross-validation to find optimal PCs
xval <- xvalDapc(tab(ps.light, NA.method = "mean"), pop(ps.light))
optimal_pca <- xval$DAPC$n.pca
print(optimal_pca)

# 2. Run DAPC with optimal parameters
ps.dapc <- dapc(ps.light, 
                   n.pca = xval$DAPC$n.pca, 
                   n.da = min(3, nlevels(pop(ps.light))-1))

# 3. Visualize
scatter(ps.dapc, scree.da = FALSE, scree.pca = TRUE)

# 4. Structure plot
compoplot(ps.dapc)


# 5. Loading plot to see which alleles contribute most
loadingplot(ps.dapc$var.contr, threshold = quantile(ps.dapc$var.contr, 0.75))
