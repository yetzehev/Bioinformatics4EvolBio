...
```{r input.file}
system( "/home/irojas/bin/plink/plink --vcf ./subset/teosintes_subset.recode.vcf --make-bed --allow-extra-chr --out ./subset/teosintes_subset ")
...
#system("bash -c 'for K in {1..7}; do ~/bin/admixture_linux-1.3.0/admixture -s time --cv=10 -j4 ./subset/teosintes_subset.bed $K | tee ./results/teosintes_subset5_${K}.out; done'")


```

```{r plot.CV}
system("grep 'CV' ./results/teosintes_subset5_*.out | sed -E 's/.*K=([0-9]+).*: ([0-9.]+).*/\\1\\t\\2/' > ./results/teosintes_clean.CV")
...
El siguiente paso es graficar el numero de cluster con el Error de Validacion Cruzada mas bajo, que en este caso es K=4.
