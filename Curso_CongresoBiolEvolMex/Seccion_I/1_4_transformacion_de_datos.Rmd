...
Nuestra base de datos se compone de solo 200 individuos; sin embargo, al correr el archivo genind nos arroja un error, esto significa que algunos de los ID de los indivuos se encuentran repetidos, antes de poder convertilos a un archivo `genind`, debemos filtrarlos:

```{r corregir IDs, warning=FALSE}
fish.vcf@fix[, 3] <- tibble(
  id = fish.vcf@fix[, 3],
  chrom = fish.vcf@fix[, 1],
  pos = fish.vcf@fix[, 2]
) %>%
  mutate(
    pos_clean = str_replace(pos, "^[^_]*__", ""),  # "1__101248__226" → "101248__226"
    new_id = if_else(str_detect(id, "NA|\\."), paste0(chrom, ":", pos_clean), id)
  ) %>%
  pull(new_id) %>%
  as.character()
```
